...
export async function runLighthouse(target: string, owner: Owner = 'website-qa-checklist', toolName = 'performance_lighthouse') {
  ...
  try {
    ...
    return evidence({ owner, tool: toolName, target, data: { categories, metrics: { fcp: audits['first-contentful-paint']?.numericValue ?? null, lcp: audits['largest-contentful-paint']?.numericValue ?? null, cls: audits['cumulative-layout-shift']?.numericValue ?? null, tbt: audits['total-blocking-time']?.numericValue ?? null, speedIndex: audits['speed-index']?.numericValue ?? null }, finalUrl, lighthouseVersion: result.lhr.lighthouseVersion }, limits: ['Lighthouse is a lab run. It is not field Core Web Vitals and does not prove production performance for all users.'] });
  } finally {
    await chrome.kill();
  }
}
